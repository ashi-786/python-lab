...
BASE_URL = "https://www.neonews.pk/latest"
...
def get_news_list():
    ...
    all_news = soup.select("div.right div.style-text")
    news_list = []

    for i, news in enumerate(all_news, 1):
        title = news.select_one("p")
    ...
    async with semaphore:
        try:
            ...
            async with session.get(article['link'], headers=headers, timeout=30,ssl=False) as res:
                ...
                article_div = soup.select_one("div.right")
                author = article_div.select_one(".zm-post-meta-author")
                print(author)
                article["author"] = author.get_text(strip=True) if author else ""
                time_text = article_div.select_one(".zm-post-meta-date")
                raw_time = time_text.get_text(strip=True) if time_text else ""
                print(raw_time)
                try:
                    dt = datetime.strptime(raw_time.strip(), "%I:%M %p, %d %B, %Y")
                    article["time"] = dt.strftime("%Y-%m-%d %H:%M:%S")
                except ValueError:
                    article["time"] = ""

                main_img = article_div.select_one("div.detail-page-image img")
                article["main_image"] = main_img['src'] if main_img else ""

                body = article_div.select_one("div.news-content")
                if body:
...
if __name__ == "__main__":
    ...
    with open('neonews_ur.json', 'w', encoding='utf-8') as f:
